...
void update_board(int x, int y, toption option)
{
    ...
    case both:
        point(scren_x, screen_y);
        printw("%c%c%c", board[x][y].enemy, board[x][y].star,
               board[x][y].tf);
        break;
    };                          /*switch ( */
}
    ...
    while (battle) {
        ...
        if (battle) {
            ...
            if ((tf[ENEMY][new_tf].t + tf[ENEMY][new_tf].s +
                ...
                tf[ENEMY][new_tf].eta =
                    (int) ((slist[dstar] - 0.01) / vel[ENEMY]) + 1;
            ...
            do {
                ...
                case 'R':
                    ...
                    break;
                case 'O':
                    display_forces(ennum, plnum, &enodds, &plodds,
                                   &battle);
                    break;
                case 'W':
            ...
            if (tf[ENEMY][new_tf].dest != 0) {
                ...
                tf[ENEMY][ennum].t =
                    tf[ENEMY][ennum].t - tf[ENEMY][new_tf].t;
                tf[ENEMY][ennum].s =
                    tf[ENEMY][ennum].s - tf[ENEMY][new_tf].s;
                tf[ENEMY][ennum].c =
                    tf[ENEMY][ennum].c - tf[ENEMY][new_tf].c;
                tf[ENEMY][ennum].b =
                    tf[ENEMY][ennum].b - tf[ENEMY][new_tf].b;
                zero_tf(ENEMY, ennum);
                display_forces(ennum, plnum, &enodds, &plodds, &battle);
            };
        };
