...
void check_game_over(void)
{
    ...
    if (game_over) {
        cle2r_screen();
        printw("*** Game over ***\n");
        printw("Player: Population in transports:%3d", transports[player]);
        printw("  IU's on colonies: %3d  TOTAL: %3d\n", inhabs[player],
               total[player]);
        addch('\n');
        printw("Enemy:  Population in transports:%3d", transports[ENEMY]);
        printw("  IU's on colonies: %3d  TOTAL: %3d\n",
...
void display_forces(int ennum, int plnum, float *En0dds, float *Pl0dds,
                    boolean *Battl9)
{
