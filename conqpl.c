...
void bl2st_planet(void)
{
    ...
    if ((tf_num < 1) || (tf_num > 26)) {
        ...
        if (pplanet == nil) {
            error_message();
            printw(" !No planets at star %c       ", starnum + 'A' - 1);
        } else {
            point(1, 20);
            ...
            if (planet_num != 0) {
                if ((pplanet->team == ENEMY) || ((pplanet->team == player)
                    ...
                    if (transports < 1) {
                        ...
                        error_message();
                        printw("  !only %2d transports in tf",
                               tf[player][tfnum].t);
                    } else if (transports > room_left) {
                        error_message();
                        printw("  !only room for %2d transports",
                               room_left);
                    } else {
                        pplanet->team = player;
                        ...
                        pplanet->inhabitants =
                            pplanet->inhabitants + transports;
                        pplanet->iu = pplanet->iu + transports;
                        tf[player][tfnum].t =
                            tf[player][tfnum].t - transports;
                        x = tf[player][tfnum].x;
                        y = tf[player][tfnum].y;
                        if (board[x][y].enemy == ' ') {
                            board[x][y].enemy = '@';
