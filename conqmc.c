...
boolean underdefended(int starnum)
{
    ...
    while ((pplanet != nil) && (!result)) {
        if ((pplanet->team == ENEMY) && (pplanet->iu > 10) &&
            ((6 * pplanet->amb + pplanet->mb) <
             conq_round(pplanet->iu / 15)))
            result = true;
        pplanet = pplanet->next;
    };
    return (result);
...
void send4transports(float slist[nstars + 1], struct sttf *task)
{
    ...
    if (task->t > 0) {
        ...
        if (best_star == task->dest) {  /*land */
            if ((tf_stars[best_star][player] == 0)
                && (best_plan->team != player)
                ) {
                trash1 = task->t;
                trash2 =
                    (best_plan->capacity - best_plan->inhabitants) / 3;
...
void send_scouts(float slist[nstars + 1], struct sttf *task)
{
    ...
    if (task->s > 0) {
        ...
        while ((doind > 0) && (task->s > 0)) {
            ...
            tf[ENEMY][new_tf].eta =
                (int) ((slist[doable[dest]] - 0.01) / vel[ENEMY])
                + 1;
            depart(task->dest);
        ...
        while (task->s > 0) {
            ...
            tf[ENEMY][new_tf].eta =
                (int) ((slist[dest] - 0.01) / vel[ENEMY]) + 1;
            depart(task->dest);
            task->s = task->s - 1;
        };
    };
